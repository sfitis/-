...
def find_pred(test_y,pred_y):
    ...
    for i in range(0, len(df)):
        if df.loc[i]['ture'] == 1:
            pred.append(df.loc[i]['1'])
        if df.loc[i]['ture'] == 0:
            pred.append(df.loc[i]['0'])
    return pred
...
class LR:
    ...
    #         #原：train_y = np.concatenate((self.predict(self.train_x), train_y), axis=0)
...
class LR_2:
    ...
    def test_model_auc(self, test_x, test_y):
        pred_y = self.model.predict_proba(self.scaler.transform(test_x))
        #pred = find_pred(test_y, pred_y)
        return roc_auc_score(test_y, pred_y)  # binary class classification AUC
        #return roc_auc_score(test_y, pred_y[:, 1], multi_class="ovr", average=None)  # multi-class AUC
