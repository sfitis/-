...
def ClientTraining0(args):  # 205 if self.original_model_name in ["lenet", 'simpleCNN']
    # 读取参数
    client = args["client"]
...
def ClientTraining1(args):  # 208 if self.original_model_name in ["LR", "LR_without"]
    # 读取参数
    client = args["client"]
    local_epoch = args["local_epoch"]
    # 读取全局参数和初始模型
...
# 233 if self.original_model_name in ['lenet', 'simpleCNN']:
...
# 237 elif self.original_model_name in ['LR', "LR_without"]:
...
class FederatedTrainer:
    ...
    def training(self, max_agg_number, model_path, local_epoch, local_batch_size, data_path, unlearning_id=None):
        ...
        for k in tqdm(range(max_agg_number), desc="Federated Learning"):
            ...
            # print(model_path)
            # print(global_parameters['coef'])
            # print(type(global_parameters['coef'])) #2维数组 numpy.ndarray
            # if(k==max_agg_number-1):
            #     print("1111111111111111111111111")

            # 保存聚合后的参数
            np.save(model_path, global_parameters)
            # print("保存地址：{}", model_path)
